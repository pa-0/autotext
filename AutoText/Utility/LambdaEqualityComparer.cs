...
{
	/// <summary>
	/// LambdaEqualityComparer - avoids the need for writing custom IEqualityComparers
	/// 
	/// Usage:
	/// 
	/// List<MyObject> x = myCollection.Except(otherCollection, new LambdaEqualityComparer<MyObject>((x, y) => x.Id == y.Id)).ToList();
	/// 
	/// or
	/// 
	/// IEqualityComparer comparer = new LambdaEqualityComparer<MyObject>((x, y) => x.Id == y.Id);
