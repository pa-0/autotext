...
namespace WindowsInput
{
	...
	public static class InputSimulator
	{
		...
		/// These left- and right-distinguishing constants are available to an application only through the GetKeyboardState, SetKeyboardState, GetAsyncKeyState, GetKeyState, and MapVirtualKey functions. 
		/// </remarks>
		public static bool IsTogglingKeyInEffect(Keys keyCode)
		{
			Int16 result = GetKeyState((UInt16)keyCode);
		...
		public static void SimulateKeyDown(Keys keyCode)
		{
			var down = new INPUT();
		...
		{
			byte[] inputStrBytes = Encoding.Unicode.GetBytes(new char[] { key });
			INPUT[] inputList = new INPUT[1];
		...
		public static void SimulateKeyUp(Keys keyCode)
		{
			var up = new INPUT();
			up.Type = (UInt32)InputType.KEYBOARD;
			up.Data.Keyboard = new KEYBDINPUT();
		...
		/// </summary>
		/// <param name="keyCode">The VirtualKeyCode to press</param>
		public static void SimulateKeyPress(Keys keyCode)
		{
			var down = new INPUT();
			down.Type = (UInt32)InputType.KEYBOARD;
			down.Data.Keyboard = new KEYBDINPUT();
			...
			uint numberOfSuccessfulSimulatedInputs = SendInput(2, inputList, Marshal.SizeOf(typeof(INPUT)));
		...
		public static void SimulateTextEntry(string text)
		{
			...
			byte[] inputStrBytes = Encoding.Unicode.GetBytes(text);
			INPUT[] inputList = new INPUT[inputStrBytes.Length * 2];

			for (int x = 0; x < inputStrBytes.Length; x += 2)
			{
				//UInt16 scanCode = chars[x];
				ushort scanCode = BitConverter.ToUInt16(inputStrBytes, x);
		...
		public static void SimulateModifiedKeyStroke(Keys keyCode, params Keys[] modifierKey)
		{
			SimulateModifiedKeyStroke(new Keys[] { keyCode }, modifierKey);
		}
		...
		public static void SimulateModifiedKeyStroke(IEnumerable<Keys> keyCodes, params Keys[] modifierKeyCodes)
		{
			if (modifierKeyCodes != null)
			{
				foreach (Keys keys in modifierKeyCodes)
				{
			...
			if (modifierKeyCodes != null)
			{
				foreach (Keys keys in modifierKeyCodes.Reverse())
				{
					SimulateKeyUp(keys);
				}
			}
